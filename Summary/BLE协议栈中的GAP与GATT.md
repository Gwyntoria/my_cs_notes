...
*属性值*可以是固定长度或可变长度。对于可变长度属性值，每个 Protocol Data Unit(PDU)中只允许发送一个属性值。如果值过长，可以将其分割成多个 PDU。
...
特征(Characteristic)是一些属性(Attribute)的集合，通常被用于表示设备的**某个特定特性或功能**。实际上，在低功耗蓝牙 GATT 层中，数据以属性的形式进行交互。
...
1. **属性句柄(Handle)：** 每个特征都有一个唯一的句柄，用于在 GATT 数据库中标识和定位该特征。
2. **UUID（通用唯一标识符）：** 表示特征的唯一标识符。UUID 用于区分不同的特征。
