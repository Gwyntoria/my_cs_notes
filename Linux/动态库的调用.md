...
可以认为系统中存在一种**引用计数机制**， 每当一个进程加载了共享库（在该进程的页表中进行一次映射），引用计数加一；一个进程显式卸载（通过`dlclose`等）共享库或进程退出时，引用计数减一，当减少到 0 时，系统卸载共享库。
...
3. 关闭动态链接库：`dlclose`，函数原型为: `int dlclose (void *handle)`; `dlclose`用于关闭指定句柄的动态链接库，只有当此动态链接库的使用计数为 0 时,才会真正被系统卸载。
