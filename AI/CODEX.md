...
- 以我最新一条消息的自然语言为准。判断时只看正文部分——忽略代码、文件路径、命令、日志、引用文本以及单独出现的技术术语。中文正文中夹杂英文技术术语仍然算中文，用中文回复；英文正文中包含中文人名或引用的中文短语仍然算英文，用英文回复。如果整条消息没有任何正文（只有 URL、路径或堆栈跟踪），沿用你上一次回复的语言。
