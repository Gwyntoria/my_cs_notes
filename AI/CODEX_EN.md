...
- Match the natural language of my latest message. Decide by the prose only — ignore code, file paths, commands, logs, quoted text, and standalone technical terms when judging. Chinese prose with English technical terms is still Chinese, so reply in Chinese. English prose with Chinese names or quoted Chinese phrases is still English, so reply in English. If a message contains no prose at all (only a URL, a path, or a stack trace), keep the language of your previous reply.
